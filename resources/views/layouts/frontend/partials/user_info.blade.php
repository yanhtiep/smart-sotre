...
<body>
    ...
    @if(!isset($user))
        <li><a id="btn-login-dark" href="{{url('/user/login')}}">Sign in &nbsp; /</a></li>
    @else
    ...
    <div class="lg-container">
        ...
        @endif
        <form action="{{url('/user-info')}}" id="lg-form" name="lg-form" method="post">
            ...
            <div>
                ...
                <input class="forms" type="email" name="email" placeholder="email" readonly value="{{$user->email}}" />
                {!! $errors->first('email', '<div class="help-block col-xs-12 col-sm-reset inline error" style="color:#ff5a92;">:message</div>') !!}
            </div>
            <div>
                <input type="checkbox" id="changePassword" name="changePassword"><span style="float: left;">Change Password:</span>
                <input class="password" type="password" name="password" id="password" aria-describedby="basic-addon1" placeholder="new password" disabled />
                {!! $errors->first('password', '<div class="help-block col-xs-12 col-sm-reset inline error" style="color:#ff5a92;">:message</div>') !!}
            </div>
            <div>
                <label for="password">Confirm Password :</label>
                <input class="forms password" type="password" name="confpassword" id="confpassword" aria-describedby="basic-addon1" placeholder="confirm password" disabled />
                {!! $errors->first('confpassword', '<div class="help-block col-xs-12 col-sm-reset inline error " style="color:#ff5a92;">:message</div>') !!}
            </div>
